...
def obtener_contrato_por_id(contrato_id:str):
...
def obtener_analisis_por_contrato(
    contrato_id:str
):
    ...
    try:
        ...
        analisis=db.query(AnalisisIA).filter(
            AnalisisIA.contrato_id==contrato_id
        ).order_by(
            AnalisisIA.fecha_analisis.desc()
        ).first()
...
def existe_analisis(contrato_id:str):

    db=SessionLocal()

    try:

        existe=db.query(AnalisisIA).filter(
            AnalisisIA.contrato_id==contrato_id
        ).first()
...
def guardar_analisis_completo(
    ...
    try:
        ...
        contrato=Contrato(
            ...
            contrato_id=contrato_secop.get(
                "id_contrato"
            ),
